...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		inoodlecallIE = $.browser.msie ? true:false;
		...
		if (inoodlecallIE && $.browser.version <= 6) {
		...
		tdc = inoodlecallIE ?  "class='ui-th-div-ie'" :"";
		...
		if(inoodlecallIE) { if(String(ts.p.height).toLowerCase() === "auto") { ts.p.height = "100%"; } }
		$(grid.bDiv)
			.append($('<div style="position:relative;'+(inoodlecallIE && $.browser.version < 8 ? "height:0.01%;" : "")+'"></div>').append('<div></div>').append(this))
