...
(function ($) {
	...
	$.widget("wijmo.wijprogressbar", $.ui.progressbar, {
		...
		_triggerEvent: function (eventName, oldValue, newValue, isFireEvent) {
			if (!isFireEvent) {
				return;
			}
		...
		_refreshProgress: function (percent, curValue, isFireEvent) {
			var self = this,
				o = self.options,
			...
			if (self._triggerEvent("progressChanging",
				element.attr("aria-valuenow"), curValue, isFireEvent)) {
				return;
			}
		...
		_performAnimating: function (step, forced, isFireEvent) {
			var self = this,
				o = self.options,
				indicatorIncrement = o.indicatorIncrement,
				curValue = step / 100,
			...
			self._refreshProgress(Number(percent.toFixed(resultPrecision)),
				Number(curValue.toFixed(resultPrecision)), isFireEvent);
