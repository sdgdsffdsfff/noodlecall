  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function init() {
			$('#filterBtn').button().click(function() {
				...
				treeLeft.buildTree({
					...
					reqUrl: 'monitor/service/tree/queryservicelist',
		...
		function treeHighlightEvent(data, label, node) {
			...
			var divId = ppppid.replace(/\W/g, '') + '_' + 
						ppid.replace(/\W/g, '') + '_' +
						id.replace(/\W/g, '');
			var title = node.parent.parent.label.substring(0, node.parent.parent.label.indexOf('(')) + '-' + 
						node.parent.label + '-' + 
						label;
				...
				if ($('#' + divId).length == 0) {			
					buildChart({
						...
						input: {
							monitorType: ppppid,
							themeName: ppid,
							selfModuleType: node.data.other,
							selfModuleId: id,
							moduleType: "DEFAULT",
							moduleId: 0
						}
		...
		function fullscreen() {
			...
			openChildWindowPost(
					'<%=request.getContextPath()%>/view/monitor/server/overtime/overtime_chart_fullscreen.jsp', 
					'urlParam',
