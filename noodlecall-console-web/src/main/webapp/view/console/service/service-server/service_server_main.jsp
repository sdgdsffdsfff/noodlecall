  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function init() {
			...
			var paramObject = {'service_Name': urlParamObject.service_Name};
			
			$('#list_left').jqGrid({
		   		url: '<%=request.getContextPath()%>/console/server/queryincludeservicepage',
				datatype: 'json',
				mtype: 'post',
				...
				colNames: [
			   	...
			   	colModel: [
					{name:'server_Id', index:'server_Id', width:80, align:'center'},
					{name:'server_Name', index:'server_Name', width:280, align:'center'},
					...
					{name:'server_Type', index:'server_Type', width:120, align:'center', formatter:'select', editoptions:{value:'JETTY:JETTY;SERVLET:SERVLET;NETTY:NETTY'}},
					{name:'serialize_Type', index:'serialize_Type', width:120, align:'center', formatter:'select', editoptions:{value:'JSON:JSON;HESSIAN:HESSIAN'}},
			...
			$('#list_right').jqGrid({
		   		url: '<%=request.getContextPath()%>/console/server/queryexcludeservicepage',
				datatype: 'json',
				mtype: 'post',
				postData:{'input': jsonToString(paramObject)}, 
				colNames: [
		...
		function inserts() {
			...
			for (var i=0; i<indexArray.length; i++) {				
				...
				ret['service_Name'] = $('#service_Name').val();
				retArray.push(ret);
			}
			...
			transaction({
				id: 'INSERTS',
				url: '<%=request.getContextPath()%>/console/server/insertsservice',
				jsonSet: jsonSet
			});	
		}
		...
		function deletes() {
			...
			transaction({
				id: 'DELETES',
				url: '<%=request.getContextPath()%>/console/server/deletesservice',
				jsonSet: jsonSet
			});	
		}
						...
						<div id="form_left" style="width: auto;">
							<table width="100%" border="1" cellspacing="0" cellpadding="0" class="s_layout">
							    ...
							    <tr>
							    	...
							    	<th><label>服务端ID</label></th>
							    	<td><input type="text" id="server_Id" maxlength="10"/></td>
							    </tr>		
							    <tr>
							    	<th><label>服务端名称</label></th>
							    ...
							    <tr>
							    	...
							    	<td><input type="text" id="url" maxlength="255"/></td>
							    </tr>		
							    <tr>
							    	<th><label>服务器类型</label></th>
							    	<td>
									...
									<th><label>序列化类型</label></th>
							    	<td>
							    		<select id="serialize_Type">
							    			<option value="">--all--</option>
