  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function init() {
			...
			$('#list').jqGrid({
		   		url: '<%=request.getContextPath()%>/console/server/querypage',
				datatype: 'local',
				mtype: 'post',
			   	colNames: [
					'服务端ID', 
		...
		function insert() {
			top.openDialog('服务端新增', '<%=request.getContextPath()%>/view/console/server/server_edit.jsp', null, 345, 700, query);
		}
		...
		function update() {
			...
			top.openDialog('服务端修改', '<%=request.getContextPath()%>/view/console/server/server_edit.jsp', ret, 345, 700, query);					
		}
		...
		function deletes() {
			...
			transaction({
				id: 'DELETES',
				url: '<%=request.getContextPath()%>/console/server/deletes',
				jsonSet: jsonSet
			});	
		}
	</script>
